...
import { AdminPriceSelectionParams } from "@medusajs/medusa/dist/types/price-selection"
import { IsType } from "@medusajs/medusa/dist/utils/validators/is-type"

/**
 * @oas [get] /variants
 * operationId: "GetVariants"
 * summary: "List Rental Variants"
 * description: "Retrieves a list of Rental Variants"
 * x-authenticated: true
 * parameters:
 *   - (query) id {string} A Rental Variant id to filter by.
 *   - (query) ids {string} A comma separated list of Rental Variant ids to filter by.
 *   - (query) expand {string} A comma separated list of Rental Variant relations to load.
 *   - (query) fields {string} A comma separated list of Rental Variant fields to include.
 *   - (query) offset=0 {number} How many rental variants to skip in the result.
 *   - (query) limit=100 {number} Maximum number of Rental Variants to return.
 ...
 *     style: form
 *     explode: false
 *     description: rental variant title to search for.
 *     schema:
 *       oneOf:
 *         - type: string
...
export default async (req, res) => {
  const variantService: RentalVariantService = req.scope.resolve(
  ...
  const variants = await pricingService.setProductPrices(rawVariants, {
