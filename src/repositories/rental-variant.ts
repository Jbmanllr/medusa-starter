...
export type FindWithRelationsOptions = FindManyOptions<RentalVariant> & {
  order?: OrderByCondition
  withDeleted?: boolean
}

@EntityRepository(RentalVariant)
export class RentalVariantRepository extends Repository<RentalVariant> {
  private mergeEntitiesWithRelations(
    entitiesAndRelations: Array<Partial<RentalVariant>>
  ): RentalVariant[] {
  ...
  private async queryRentalsVariants(
  ...
  private getGroupedRelations(relations: Array<keyof RentalVariant>): {
  ...
  private async queryRentalVariantsWithIds(
    entityIds: string[],
  ...
  public async findWithRelationsAndCount(
    ...
    if (Array.isArray(idsOrOptionsWithoutRelations)) {
      ...
      const result = await this.queryRentalsVariants(
    ...
    if (relations.length === 0) {
      const toReturn = await this.findByIds(
        entitiesIds,
        idsOrOptionsWithoutRelations as FindConditions<RentalVariant>
      )
      return [toReturn, toReturn.length]
    }

    const groupedRelations = this.getGroupedRelations(
      relations as (keyof RentalVariant)[]
    )
    const entitiesIdsWithRelations = await this.queryRentalVariantsWithIds(
      entitiesIds,
      groupedRelations,
      withDeleted
    )
  ...
  public async findOneWithRelations(
    relations: Array<keyof RentalVariant> = [],
